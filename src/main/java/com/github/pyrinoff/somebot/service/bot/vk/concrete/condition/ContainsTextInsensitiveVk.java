...
public class ContainsTextInsensitiveVk<U extends User, M extends AbstractVkMessage<U>>
        implements AbstractVkCondition<U, M> {

    final String theText;

    public ContainsTextInsensitiveVk(final String text) {
        this.theText = text;
    }
    ...
    public boolean isFired(final M message) {
        return message.getText() != null && !message.getText().isEmpty()
                && message.getText().toLowerCase().contains(theText.toLowerCase())
                ;
    }
